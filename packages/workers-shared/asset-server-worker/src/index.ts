...
export default class extends WorkerEntrypoint<Env> {
	...
	private async getAssetEntry(request: Request) {
		...
		// this.env.ASSETS_MANIFEST is a pipeline binding to an ArrayBuffer containing the
	...
	private async handleRequest(request: Request) {
		...
		if (!assetResponse || !assetResponse.value) {
			throw new Error(
				`Requested asset ${assetEntry} existed in the manifest but not in the KV namespace.`
			);
		}
